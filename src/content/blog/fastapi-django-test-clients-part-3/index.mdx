...
But why `__call__`? What it gives us? Also it is not really clear how this logic in run for every test, not just once the test case. In depth explanation will take us away to far from the main topic. Let's keep it very simple, even oversimplified. You can run test by calling test case instance.
...
return result
```
Again, this is huge oversimplification and not how things works under the hood, but important thing to remember is test case instance is called once per test, not per test case.
...
Now we have pretty good insight on how Django how django manager database transactions in tests. We also now understand that many pieces has to fall in right places to enable transaction wrapping in tests. Even with all that, this solution isn't perfect.
