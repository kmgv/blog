...
Django uses `@transaction.atomic` (<Source source="web" path="https://docs.djangoproject.com/en/6.0/topics/db/transactions/" link="docs" />) decorator to wrap piece of code in transaction. `setUpClass()` creates an instance of the same `atomic` class (although not used as decorator). **So each test runs inside a transaction!** Keep in mind that it loads fixtures after calling `_enter_atomics()` .
...
I want you to take a moment and think how many pieces have to fall in place like puzzles in order to make it possible.

* Django uses the same database connection shared across tests, view functions and any other place.
* connection and transaction are (almost) never explicitly handled by the developer. This prevents the developer from committing a transaction that is meant to be rolled back later in tests.
* `transaction.atomic` can be nested without committing or rolling back whole session - just to a savepoint.
* ORM is integral part of Django so systems (like testing and database connections handling) can work together seamlessly out of the box

As you can see there are multiple architecture decisions, that allowed to create abstractions to seamlessly deliver database cleanup by wrapping it in transaction. We started with test client to show that seemingly irrelevant part plays its role here.
...
## Appendix
I want to leave you with a couple of hints how to figure out how database connection is shared.

* We already saw atomic uses `django.db.connections` <Source path="django/db/transaction.py:1" source="django" sources={sources} link="here" />
* Digging in `QuerySet` (`QuerySet` is what you get when calling `Model.objects.all()`) we stumble upon functions responsible for generating queries like <Source path="django/db/models/query.py:1893" source="django" sources={sources} link="_insert()" />
* Look how `Query.get_compiler` gets connection, <Source path="django/db/models/sql/query.py:372" source="django" sources={sources} link="here" />

See something similar? ;)

There are always more questions and more digging to be done. How are those connections initialized? How to show there is only one at the same time? Always more to discover.