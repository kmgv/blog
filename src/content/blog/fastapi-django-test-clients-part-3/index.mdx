...
Transaction are SQL specific, so it is a mechanism provided by database, not by Django, SQLALchemy or any other python package. Transaction is like a bag for queries. At the end of transaction we can save all changes or discard them. The most important thing is all actions either succeeds, fail or are discarded altogether. Only after transaction is committed (saved) data is saved to the database.
...
One thing important to notice, savepoints don't give us ability to commit transaction multiple times. When we call `COMMIT` whole session get committed, not the last savepoint.
...
How do we know this is a good place to start? This test creates some database entries in setup and have access to them in test. We know that Django cleans up database between every test, and there no other setup required mentioned by docs. All "magic" has to be happening inside `TestClass`.

Before we delve magic world on django internal we have to address few thing first so we don't feel lost later.
...
**setUpClass()** - classmethod that runs once before all test in the test case<br/>
**TearDownClass()** - classmethod that runs once after all test in the test case<br/>
...
Some of you may say, "ok but I still don't see how this logic is run for each test, not once for whole suite". Well, we are getting too far away from main topic. I'll send you back to the <Source source="web" path="https://docs.python.org/3/library/unittest.html#unittest.TestCase.run" link="docs" /> but also provide very oversimplified answer.
...
* thanks to architecture of Django and it's test client tests and application can and does share same database connection
* in django fixtures are extra data loaded to database before tests. Used for populating db with things like constants values or user account used in all tests.

### Class setup and teardown.

Lets start with `TestCase` to check if does any setup for whole test case.
...
Django uses `@transaction.atomic` (<Source source="web" path="https://docs.djangoproject.com/en/6.0/topics/db/transactions/" link="docs" />) decorator to wrap piece of code in transaction. `setUpClass()` creates same `atomic` instance (although not used as decorator). **So each test runs inside a transaction!** Keep in mind that it loads fixtures after calling `_enter_atomics()` .
...
Yes! It marks transaction to rollback and exists atomic that was set up in `setUpClass`. Let's sum up what is going on in `TestCase` so far:

1. `setUpClass` starts transaction for whole test case. Keeps related `Atomic` in `cls.atomics`.
2. loads fixures
3. runs tests
4. marks transaction to be rolled back and exist `Atomic` from point 1
...
Most of developer probably never had to think about that, but Django's default behavior is autocommit (<Source source="web" path="https://docs.djangoproject.com/en/6.0/topics/db/transactions/#django-s-default-transaction-behavior" />). What that means? Without using `@transaction.atomic` you get experience similar to using you database cli. Each query resulting in changes is immediately saved to DB without ability to undo. Remember last time you updated to many rows because you forgot `WHERE` clause? Yeah, that behavior.

On the other hand with `autocommit=False` database driver implicitly start new transaction by injecting `BEGIN` before your query. More on that can be found at the top of <Source source="web" path="https://www.psycopg.org/psycopg3/docs/basic/transactions.html" link="docs for postgres driver"/> and in <Source source="web" path="https://www.psycopg.org/psycopg3/docs/basic/transactions.html#autocommit-transactions" link="autocommit section"/>
...
**`get|set_autocommit()`** - get/set autocommit behavior we talks about minute ago <br/>
...
1. create a savepoint and append it connection's savepoints list


We have pretty good grasp what happens when we enter `Atomic`. Lets look at exit now.
...
2. If we are not longer in atomic block, we either commit or rollback whole transaction.
3. Else, if connection in marked to rollback we rollback only to the last savepoint.
...
3. After each test `TestCase` marks transaction to rollback and exits `transaction.atomic`. It rolls back transaction to the savepoint before test and thus cleans all changes make during test.
...
Now we have pretty good insight on how Django how django manager database transactions in tests. We also now understand that many pieces has to fall in right places to enable transaction wrapping in tests. Even with all that, this solution it perfect.

In next part we will look at what options we have with FastAPI and SQLAlchemy and do some benchmarks. We'll also show why Django's approach is not easily to recreate there. Don't worry, we are not leaving `TransactionTestCase` for good. We will do through it's cleanup mechanisms in relation to other frameworks.
